...
class MultiVecTaskPython(MultiVecTask):
    ...
    def step(self, actions):
        ...
        hand_obs = []
        obs_buf = torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(
            self.rl_device
        )
        hand_obs.append(
            torch.cat(
...
class SingleVecTaskPythonArm:
    def __init__(
        self, task: BaseTask, rl_device, clip_observations=5.0, clip_actions=1.0
    ):
        self.task = task
        self.rl_device = rl_device
        self.num_agents = 1
        ...
        self.act_space = [
            spaces.Box(
                low=task.franka1_dof_lower_limits, high=task.franka1_dof_lower_limits
            )
            for _ in range(self.num_agents)
        ]
