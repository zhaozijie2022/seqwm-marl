...
from seqwm.envs.quadruped.mqe.utils.terrain import get_terrain_cls
...
class LeggedRobotField(LeggedRobot):
    ...
    def _create_onboard_camera(self, env_handle, actor_handle, sensor_name):
        ...
        if hasattr(getattr(self.cfg.sensor, sensor_name), "horizontal_fov"):
            ...
            ) if isinstance(getattr(self.cfg.sensor, sensor_name).horizontal_fov, (tuple, list)) else getattr(self.cfg.sensor,
                                                                                                              sensor_name).horizontal_fov
    ...
    def check_termination(self):
        ...
        if "roll" in self.cfg.termination.termination_terms:
            self.r_term_buff = (torch.abs(r) > self.cfg.termination.roll_kwargs["threshold"]).reshape(self.num_envs, -1).sum(1).to(
                torch.bool)
            self.reset_buf |= self.r_term_buff

        if "pitch" in self.cfg.termination.termination_terms:
            self.p_term_buff = (torch.abs(p) > self.cfg.termination.pitch_kwargs["threshold"]).reshape(self.num_envs, -1).sum(1).to(
                torch.bool)
            ...
            if hasattr(self.cfg.control, "torque_limits"):
                if not isinstance(self.cfg.control.torque_limits, (tuple, list)):
                    ...
                    self.torque_limits = torch.tensor(
                        self.cfg.control.torque_limits * (self.num_actuated_dof // len(self.cfg.control.torque_limits)), dtype=torch.float,
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if hasattr(self, "robot_config_buffer"):
            ...
            self.robot_config_buffer[agent_id, 5: 5 + 12] = self.motor_strength[env_id].reshape(self.num_agents, -1) if hasattr(self,
