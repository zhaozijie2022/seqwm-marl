...
from seqwm.envs.quadruped.mqe import LEGGED_GYM_ROOT_DIR, envs
from seqwm.envs.quadruped.mqe.envs.base.legged_robot import LeggedRobot
from seqwm.envs.quadruped.mqe.envs.field.legged_robot_field import LeggedRobotField
from seqwm.envs.quadruped.mqe.envs.go1.go1_config import Go1Cfg
from seqwm.envs.quadruped.mqe.utils.math import quat_apply_yaw, wrap_to_pi, torch_rand_sqrt_float
from seqwm.envs.quadruped.mqe.utils.helpers import class_to_dict
...
class Go1(LeggedRobotField):
    def __init__(self, cfg: Go1Cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.last_locomotion_action = torch.zeros(self.num_envs * self.num_agents, 12, dtype=torch.float, device=self.device,
                                                  requires_grad=False)
        self.last_two_locomotion_action = torch.zeros(self.num_envs * self.num_agents, 12, dtype=torch.float, device=self.device,
                                                      requires_grad=False)
    ...
    def step(self, action):
        ...
        for dec_i in range(self.decimation):
            ...
            torques = torch.cat((self.torques, torch.zeros((self.num_envs, self.num_actions_npc), dtype=torch.long, device=self.device)),
                                dim=1) if self.num_actions_npc != 0 else self.torques
    ...
    def compute_observations(self):
        ...
        if self.cfg.obs.cfgs.clock_inputs or self.cfg.control.control_type == "C":
            assert self.cfg.control.control_type == "C", "To active clock_inputs, control_type should be set to \"C\" instead of \"{}\"".format(
    ...
    def _prepare_locomotion_policy(self):
        ...
        self.history_locomotion_obs = torch.zeros(self.num_envs * self.num_agents, 2100, dtype=torch.float, device=self.device,
                                                  requires_grad=False)
    ...
    def _fill_command_obs(self):
        ...
        if not self.cfg.command.cfg.gait:
            ...
            locomotion_obs[0, 10] = self.cfg.command.gaits[self.cfg.control.default_command.gait][
                                        2] * self.cfg.control.obs_scales.gait_phase
    ...
    def _init_custom_buffers__(self):
        ...
        self.doubletime_clock_inputs = torch.zeros(self.num_envs * self.num_agents, 4, dtype=torch.float, device=self.device,
                                                   requires_grad=False)
        self.halftime_clock_inputs = torch.zeros(self.num_envs * self.num_agents, 4, dtype=torch.float, device=self.device,
                                                 requires_grad=False)
