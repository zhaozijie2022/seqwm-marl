...
class Go1Sheep(Go1):
    ...
    def _prepare_npc(self):
        ...
        sheep_origin = np.array([self.cfg.terrain.BarrierTrack_kwargs["init"]["block_length"] +
                                 self.cfg.terrain.BarrierTrack_kwargs["plane"]["block_length"] / 2
    ...
    def _create_npc(self, env_handle, env_id):
        ...
        for i in range(self.num_npcs):
            npc_handle = self.gym.create_actor(env_handle, self.asset_npc, self.start_pose_npc, self.cfg.asset.name_npc, env_id,
                                               not self.npc_collision)
