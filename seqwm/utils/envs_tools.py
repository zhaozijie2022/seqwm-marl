...
import numpy as np
import torch
from seqwm.envs.env_wrappers import ShareSubprocVecEnv, ShareDummyVecEnv
...
def make_train_env(env_name, seed, n_threads, env_args):
    ...
    if env_name == "dexhands":
        from seqwm.envs.dexhands.dexhands_env import DexHandsEnv
        return DexHandsEnv({"n_threads": n_threads, **env_args})

    if env_name == "quadruped":
        from seqwm.envs.quadruped.quadruped_env import QuadrupedEnv

        return QuadrupedEnv({"n_threads": n_threads, **env_args})

    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                from seqwm.envs.smac.StarCraft2_Env import StarCraft2Env

                env = StarCraft2Env(env_args)
            elif env_name == "smacv2":
                from seqwm.envs.smacv2.smacv2_env import SMACv2Env

                env = SMACv2Env(env_args)
            elif env_name == "mamujoco":
                from seqwm.envs.mamujoco.multiagent_mujoco.mujoco_multi import (
                    MujocoMulti,
                )

                env = MujocoMulti(env_args=env_args)
            elif env_name == "pettingzoo_mpe":
                from seqwm.envs.pettingzoo_mpe.pettingzoo_mpe_env import (
                    PettingZooMPEEnv,
                )
                ...
                from seqwm.envs.gym.gym_env import GYMEnv

                env = GYMEnv(env_args)
            elif env_name == "football":
                from seqwm.envs.football.football_env import FootballEnv

                env = FootballEnv(env_args)
            elif env_name == "lag":
                from seqwm.envs.lag.lag_env import LAGEnv
...
def make_render_env(env_name, seed, env_args):
    ...
    if env_name == "mamujoco":
        from seqwm.envs.mamujoco.multiagent_mujoco.mujoco_multi import MujocoMulti
        ...
        env = QuadrupedEnv({"n_threads": 1, **env_args})
        manual_render = False  # dexhands renders automatically
